...
#define PROID			"Q90TtP6cvw" //产品ID

#define ACCESS_KEY		"z08zk7EtWekTBasBaae5Y1uDJ7Ola+c5zweRC4vBJ3c="  //设备密钥

#define DEVICE_NAME		"one"  //设备名称
...
uint8_t auto_manual_mode = 3;		//判断自动还是手动模式,auto_manual_mode：2--手动、3--自动
uint8_t key_val = 0;						//这个地方要改，当前只是只有当订阅到数据后才会进行改变，它应随着LED状态改变，因为它要实时显示LED的状态，
																//可以在onenet.c中写一个获取当前LED状态的函数，然后输出状态

extern unsigned char esp8266_buf[512];
extern u8 temperature,humidity,temperature_temp,humidity_temp;
/*
		功能		sign需要进行URL编码
		输入		sign：加密结果
		输出		0-成功	其他-失败
...
/*
		功能		计算Authorization，相当于连接新版onenet时的密钥
		输入		ver：参数组版本号，日期格式，目前仅支持格式"2018-10-31"
						res：产品id			et：过期时间，UTC秒值		access_key：访问密钥				dev_name：设备名
						authorization_buf：缓存token的指针				authorization_buf_len：缓存区长度(字节)
		输出		0-成功	其他-失败
		说明		当前仅支持sha1
*/
#define METHOD		"sha1"
...
/*
		功能		在产品中注册一个设备
		输入		access_key：访问密钥		pro_id：产品ID		serial：唯一设备号		devid：保存返回的devid		key：保存返回的key
		输出		0-成功		1-失败
*/
...
/*
		功能		将数据以cJSON的形式的形式打包，并将cjson的数据转移到buf指针所指的地址
		输入		上传的包
		输出		包的长度
*/
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Key\":{\"value\":%d},",key_val);		//开灯表示1，发送的则是true
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *raw_json,*params_json,*LED_json,*ID_json;
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				raw_json = cJSON_Parse(req_payload);									//得到平台下发的cjson整体格式
				params_json = cJSON_GetObjectItem(raw_json,"params");	//提取params这个字符
				LED_json = cJSON_GetObjectItem(params_json,"Key");		//提取Key标识符
				ID_json = cJSON_GetObjectItem(raw_json,"id");	
				
				if(LED_json != NULL)							//auto_manual_mode：2--手动、3--自动
				{
					
					char topic_buf[56];
					char *topic_tmp = topic_buf;			//返回消息内容
					char msg_buf[100];
					char *msg_tmp = msg_buf;
					
					//在设置设备属性时，是软件通过发布到 X处，再由stm32订阅x的订阅地址，$sys/%s/%s/thing/property/set
					//软件经过设置设备属性的API（这是由HTTP通信的）发布数据，32收到订阅的消息后进入到了这个函数中，再提取
					//到"Key"这个字符串后就进入了这个if语句
					//特别注意一点: 设置设备属性到32后32必须要经过 设备属性设置响应 这个通信主题给云平台一个响应，不然命令下发后平台
					//会显示设置超时，但是数据还是发到下位机了，如果不在意是否超时，把 “之间” 的注释到就行
					//返回的响应有固定格式
					/**
								{
									"id": "9",					这个id是每次发送数据会自带一个id，每次都不相同，所以取ID_json->valuestring其中的数据
									"code": 200,
									"msg": "success"
								}
					**/
					...
					UsartPrintf(USART_DEBUG,"%s",ID_json);
					...
					memset(text, 0, sizeof(text));
					sprintf(text, "\"id\":\"%s\",", ID_json->valuestring);
					strcat(msg_buf, text);
					
					UsartPrintf(USART_DEBUG,"%s",msg_buf);
					...
					OneNET_Publish(topic_tmp,msg_tmp);
					
					//之间
					
					UsartPrintf(USART_DEBUG,"data = ---- %d\r\n",LED_json->type);
					if(LED_json->valueint == 1)	//1
					{
						key_val = 1;
						GPIO_SetBits(GPIOA,GPIO_Pin_12);
						UsartPrintf(USART_DEBUG,"开灯\r\n");
					}
					
					//因为传过来的是"params":{"Key":0}这种形式	LED_json已经找到了"Key"了，valueint指的是Key之后下面的数字
					//所以就找到了1或者0，valuestring而这个是相邻的字符串，在返回设备属性设置响应用到了
					
					else if(LED_json->valueint == 0)	
					{
						key_val = 0;
						GPIO_ResetBits(GPIOA,GPIO_Pin_12);	
						UsartPrintf(USART_DEBUG,"关灯\r\n");
					}	
						free(topic_tmp);
						free(msg_tmp);
				}
