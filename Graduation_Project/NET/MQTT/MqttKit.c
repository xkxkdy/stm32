...
/*
		功能		申请内存
		输入		edpPacket：包结构体		size：大小
		输出		无
		说明		1.可使用动态分配来分配内存		2.可使用局部或全局数组来指定内存
*/
...
/*
		功能		连接消息组包
		输入		user：用户名：产品ID		password：密码：鉴权信息或apikey		devid：设备ID
						cTime：连接保持时间		clean_session：离线消息清除标志		qos：重发标志
						will_topic：异常离线topic		will_msg：异常离线消息		will_retain：消息推送标志	
						mqttPacket：包指针
		输出		0-成功		其他-失败
*/
...
/*
		功能		数据点上传组包
		输入		pro_id：产品ID(可为空)		dev_name：设备名		send_buf：json缓存buf
						send_len：json总长				type_bin_head：bin文件的消息头
		输出		0-成功		1-失败
*/
...
/*
		功能		Subscribe消息组包
		输入		pkt_id：pkt_id		qos：消息重发次数	topics：订阅的消息
						topics_cnt：订阅的消息个数		mqttPacket：包指针
		输出		0-成功		其他-失败
*/
...
/*
		功能		Pulish消息组包
		输入		pkt_id：pkt_id		topic：发布的topic		payload：消息体		payload_len：消息体长度
						qos：重发次数			retain：离线消息推送		own：				mqttPacket：包指针
		输出		0-成功		其他-失败
*/
...
/*
		功能		Publish消息解包
		输入		flags：MQTT相关标志信息		pkt：指向可变头部
						size：固定头部中的剩余长度信息
		输出		0-成功		其他-失败原因
*/
