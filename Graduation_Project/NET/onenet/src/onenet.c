...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Lux\":{\"value\":%d},", Light_ADValue);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Key\":{\"value\":%s},",LED_Status?"true":"false");		//开灯表示1，发送的则是true
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Mode\":{\"value\":%d}",auto_manual_mode);
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *raw_json,*params_json,*LED_json,*Mode_json;
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				Mode_json = cJSON_GetObjectItem(params_json,"Mode");	//mode=2--手动、mode=3--自动
				
				if(LED_json != NULL || Mode_json != NULL)							//auto_manual_mode：2--手动、3--自动
				{
					UsartPrintf(USART_DEBUG,"data = ---- %d\r\n",LED_json->type);
					if(LED_json->type == cJSON_True)
					{
						key_val = 1;
						auto_manual_mode = 2;				//手动模式
					...
					else if(Mode_json->type == 2)
					{
						UsartPrintf(USART_DEBUG,"手动模式已开启\r\n");
						auto_manual_mode = 2;
						
					}
					else if(Mode_json->type == cJSON_Number)
					{
						UsartPrintf(USART_DEBUG,"自动模式已开启\r\n");
						auto_manual_mode = 3;
